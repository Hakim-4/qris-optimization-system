...
RABBITMQ_PUBLISH_TOTAL = Counter(
    "qris_rabbitmq_publish_total",
    "Total RabbitMQ publish attempts",
    ["queue", "result"],
)

RABBITMQ_CONSUME_TOTAL = Counter(
    "qris_rabbitmq_consume_total",
    "Total RabbitMQ consumed messages",
    ["queue", "result"],
)

ASYNC_PAYMENT_TOTAL = Counter(
    "qris_async_payment_total",
    "Total async payment jobs by final status",
    ["status"],
)
...
def update_cache_hit_ratio() -> None:
    hits = _counter_value(CACHE_HITS)
    misses = _counter_value(CACHE_MISSES)
    total = hits + misses
    if total > 0:
        CACHE_HIT_RATIO.set(hits / total)


def record_cache_result(is_hit: bool) -> None:
    if is_hit:
        CACHE_HITS.inc()
    else:
        CACHE_MISSES.inc()
